...
const Login = () => {
  ...
  const handleLog = async () => {
    ...
    try {
      ...
      setPopup({ show: true, success: true, message: 'Login successfully!' });
    ...
    if (role === "user") {
      ...
      setPopup({ show: true, success: false, message: 'Login Failed !' });
    }finally{
      setLoading(false);
      hidePopup()
    }
    ...
    <div style={{ height: '100vh', position: 'relative', overflow: 'hidden' }}>
      ...
      >
        <h3 className="text-center mb-4" id="logs_head">Login</h3>
          ...
          <button className="btn btn-primary  w-100" onClick={handleLog}  disabled={loading}>
                      {loading ? (
                <>
